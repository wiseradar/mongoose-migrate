...
Set.prototype.save = function(fn){
  ...
  var path = process.env.NODE_MONGOOSE_MIGRATIONS_CONFIG || './config/migrations.js';
  ...
  fs.readFile(path, 'utf8', function(err, json){
    ...
    mongoose.connect(config.db || process.env.MONGOLAB_URI || process.env.MONGOHQ_URL, function () {
